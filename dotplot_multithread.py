...
# Generar una parte del dotplot usando múltiples hilos
...
# Generar el dotplot completo con múltiples hilos
def generar_dotplot_multihilo(seq1, seq2, num_threads=4):
    len_seq1 = len(seq1)
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        resultados = list(executor.map(generar_dotplot_parcial, tasks))
...
# Función principal
def main():
    parser = argparse.ArgumentParser(description="Generar y filtrar un dotplot entre dos secuencias usando múltiples hilos.")
    ...
    start_time = time.time()
    matriz_dotplot = generar_dotplot_multihilo(secuencia1, secuencia2, num_threads=args.threads)
